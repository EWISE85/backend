...
public class RankService : IRankService
{
	...
	public async Task<double> CalculateUserRankImpactAsync(User user, Products product)
	{
		if (product == null || user == null) return 0;
		...
		if (product.ProductValues != null && product.ProductValues.Any())
		{
			foreach (var pv in product.ProductValues)
			{
				if (pv.AttributeOptionId.HasValue && pv.Attribute?.AttributeOptions != null)
				{
					...
					if (matchedOption?.EstimateWeight.HasValue == true)
					{
						actualWeight = matchedOption.EstimateWeight.Value;
						break;
					}
		...
		if (actualWeight <= 0)
		{
			actualWeight = category?.DefaultWeight > 0 ? category.DefaultWeight : 1.0;
		}

		double factor = category?.ParentCategory?.EmissionFactor ?? category?.EmissionFactor ?? 0.5;
		double co2Saved = actualWeight * factor;
		...
		var allRanks = await _unitOfWork.Ranks.GetAllAsync();
		var oldRank = allRanks.FirstOrDefault(r => r.RankId == user.CurrentRankId);
		string oldRankName = oldRank?.RankName ?? "Thành viên mới";
