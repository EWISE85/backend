...
namespace ElecWasteCollection.Application.Services
{
    public class GroupingService : IGroupingService
    {
        ...
        public async Task<PreAssignResponse> PreAssignAsync(PreAssignRequest request)
        {
            ...
            {
                bool isSelected = request.ProductIds.Contains(p.Product.ProductId);

                bool isCorrectDate = false;
                if (TryParseScheduleInfo(p.ScheduleJson!, out var sch))
                {
                    isCorrectDate = sch.SpecificDates.Contains(targetDate);
                }

                return isSelected || isCorrectDate;
            }).ToList();
       ...
       int limit)
        ...
        public async Task<List<Vehicles>> GetVehiclesBySmallPointAsync(string smallPointId, DateOnly workDate)
        {
            var assignedVehicleIds = (await _unitOfWork.Shifts.GetAllAsync(s =>
                s.WorkDate == workDate &&
                !string.IsNullOrEmpty(s.Vehicle_Id)))
                .Select(s => s.Vehicle_Id) 
                .Distinct()
                .ToList();
            ...
            var list = await _unitOfWork.Vehicles.GetAllAsync(v =>
                ...
                !assignedVehicleIds.Contains(v.VehicleId)); 
