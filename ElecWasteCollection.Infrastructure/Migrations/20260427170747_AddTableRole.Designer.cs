// <auto-generated />
using System;
using System.Collections.Generic;
using ElecWasteCollection.Infrastructure.Context;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace ElecWasteCollection.Infrastructure.Migrations
{
    [DbContext(typeof(ElecWasteCollectionDbContext))]
    [Migration("20260427170747_AddTableRole")]
    partial class AddTableRole
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.5")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("CollectionOffDay", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("CollectionUnitId")
                        .HasColumnType("text");

                    b.Property<string>("CompanyId")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateOnly>("OffDate")
                        .HasColumnType("date");

                    b.Property<string>("Reason")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("CollectionUnitId");

                    b.HasIndex("CompanyId");

                    b.HasIndex("OffDate");

                    b.ToTable("CollectionOffDays");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Account", b =>
                {
                    b.Property<Guid>("AccountId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("IsFirstLogin")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("PasswordHash")
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<string>("Username")
                        .HasColumnType("text");

                    b.HasKey("AccountId");

                    b.HasIndex("UserId");

                    b.ToTable("Account", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.AttributeOptions", b =>
                {
                    b.Property<Guid>("OptionId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("AttributeId")
                        .HasColumnType("uuid");

                    b.Property<double?>("EstimateVolume")
                        .HasColumnType("double precision");

                    b.Property<double?>("EstimateWeight")
                        .HasColumnType("double precision");

                    b.Property<string>("OptionName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("OptionId");

                    b.HasIndex("AttributeId");

                    b.ToTable("AttributeOptions", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Attributes", b =>
                {
                    b.Property<Guid>("AttributeId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("AttributeId");

                    b.ToTable("Attributes", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Brand", b =>
                {
                    b.Property<Guid>("BrandId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("BrandId");

                    b.ToTable("Brand", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.BrandCategory", b =>
                {
                    b.Property<Guid>("BrandCategoryId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("BrandId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("CategoryId")
                        .HasColumnType("uuid");

                    b.Property<double>("Points")
                        .HasColumnType("double precision");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("BrandCategoryId");

                    b.HasIndex("BrandId");

                    b.HasIndex("CategoryId");

                    b.ToTable("BrandCategory", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Category", b =>
                {
                    b.Property<Guid>("CategoryId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AiRecognitionTags")
                        .HasColumnType("text");

                    b.Property<double>("DefaultWeight")
                        .HasColumnType("double precision");

                    b.Property<double>("EmissionFactor")
                        .HasColumnType("double precision");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid?>("ParentCategoryId")
                        .HasColumnType("uuid");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("CategoryId");

                    b.HasIndex("ParentCategoryId");

                    b.ToTable("Category", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CategoryAttributes", b =>
                {
                    b.Property<Guid>("CategoryId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("AttributeId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("CategoryAttributeId")
                        .HasColumnType("uuid");

                    b.Property<double?>("MaxValue")
                        .HasColumnType("double precision");

                    b.Property<double?>("MinValue")
                        .HasColumnType("double precision");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Unit")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("CategoryId", "AttributeId");

                    b.HasIndex("AttributeId");

                    b.ToTable("CategoryAttributes", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CollectionGroups", b =>
                {
                    b.Property<int>("CollectionGroupId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("CollectionGroupId"));

                    b.Property<DateTime>("Created_At")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Group_Code")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Shift_Id")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("CollectionGroupId");

                    b.HasIndex("Shift_Id");

                    b.ToTable("CollectionGroups", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CollectionRoutes", b =>
                {
                    b.Property<Guid>("CollectionRouteId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<TimeOnly?>("Actual_Time")
                        .HasColumnType("time without time zone");

                    b.Property<DateOnly>("CollectionDate")
                        .HasColumnType("date");

                    b.Property<int>("CollectionGroupId")
                        .HasColumnType("integer");

                    b.PrimitiveCollection<List<string>>("ConfirmImages")
                        .HasColumnType("text[]");

                    b.Property<double>("DistanceKm")
                        .HasColumnType("double precision");

                    b.Property<TimeOnly>("EstimatedTime")
                        .HasColumnType("time without time zone");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<string>("RejectMessage")
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("CollectionRouteId");

                    b.HasIndex("CollectionGroupId");

                    b.HasIndex("ProductId");

                    b.ToTable("CollectionRoutes", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CollectionUnit", b =>
                {
                    b.Property<string>("CollectionUnitId")
                        .HasColumnType("text");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("CompanyId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("Created_At")
                        .HasColumnType("timestamp with time zone");

                    b.Property<double>("CurrentCapacity")
                        .HasColumnType("double precision");

                    b.Property<double>("Latitude")
                        .HasColumnType("double precision");

                    b.Property<double>("Longitude")
                        .HasColumnType("double precision");

                    b.Property<double>("MaxCapacity")
                        .HasColumnType("double precision");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("OpenTime")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double>("PlannedCapacity")
                        .HasColumnType("double precision");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("Updated_At")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("CollectionUnitId");

                    b.HasIndex("CompanyId");

                    b.HasIndex("Created_At");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("CollectionUnits", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Company", b =>
                {
                    b.Property<string>("CompanyId")
                        .HasColumnType("text");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("CompanyEmail")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("CompanyType")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("Created_At")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("IsFallback")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("Priority")
                        .HasColumnType("integer");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("Updated_At")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("CompanyId");

                    b.HasIndex("Created_At");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("Company", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CompanyRecyclingCategory", b =>
                {
                    b.Property<string>("CompanyId")
                        .HasColumnType("text");

                    b.Property<Guid>("CategoryId")
                        .HasColumnType("uuid");

                    b.HasKey("CompanyId", "CategoryId");

                    b.HasIndex("CategoryId");

                    b.ToTable("CompanyRecyclingCategory", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.ForgotPassword", b =>
                {
                    b.Property<Guid>("ForgotPasswordId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ExpireAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("OTP")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("ForgotPasswordId");

                    b.HasIndex("UserId");

                    b.ToTable("ForgotPassword", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Image", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AiDetectedLabelsJson")
                        .HasColumnType("text");

                    b.Property<string>("ImageUrl")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("PostId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ProductId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("PostId");

                    b.HasIndex("ProductId");

                    b.ToTable("Images", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Notifications", b =>
                {
                    b.Property<Guid>("NotificationId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Body")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("EventId")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsRead")
                        .HasColumnType("boolean");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("NotificationId");

                    b.HasIndex("UserId");

                    b.ToTable("Notification", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.PackageStatusHistory", b =>
                {
                    b.Property<Guid>("PackageStatusHistoryId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ChangedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("PackageId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("StatusDescription")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("PackageStatusHistoryId");

                    b.HasIndex("PackageId");

                    b.ToTable("PackageStatusHistory", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Packages", b =>
                {
                    b.Property<string>("PackageId")
                        .HasColumnType("text");

                    b.Property<string>("CollectionUnitId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreateAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("DeliveryHandoverAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("DeliveryQrCode")
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("PackageId");

                    b.HasIndex("CollectionUnitId");

                    b.ToTable("Packages", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.PointTransactions", b =>
                {
                    b.Property<Guid>("PointTransactionId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Desciption")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double>("Point")
                        .HasColumnType("double precision");

                    b.Property<Guid?>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<string>("TransactionType")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("VoucherId")
                        .HasColumnType("uuid");

                    b.HasKey("PointTransactionId");

                    b.HasIndex("ProductId");

                    b.HasIndex("UserId");

                    b.HasIndex("VoucherId");

                    b.ToTable("PointTransactions", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Post", b =>
                {
                    b.Property<Guid>("PostId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Address")
                        .HasColumnType("text");

                    b.Property<string>("AssignedCollectionUnitId")
                        .HasColumnType("text");

                    b.PrimitiveCollection<List<string>>("CheckMessage")
                        .HasColumnType("text[]");

                    b.Property<string>("CompanyId")
                        .HasColumnType("text");

                    b.Property<DateTime>("Date")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double?>("DistanceToPointKm")
                        .HasColumnType("double precision");

                    b.Property<double>("EstimatePoint")
                        .HasColumnType("double precision");

                    b.Property<string>("RejectMessage")
                        .HasColumnType("text");

                    b.Property<string>("ScheduleJson")
                        .HasColumnType("text");

                    b.Property<Guid>("SenderId")
                        .HasColumnType("uuid");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("PostId");

                    b.HasIndex("AssignedCollectionUnitId");

                    b.HasIndex("CompanyId");

                    b.HasIndex("SenderId");

                    b.ToTable("Post", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.ProductStatusHistory", b =>
                {
                    b.Property<Guid>("ProductStatusHistoryId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ChangedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("StatusDescription")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("ProductStatusHistoryId");

                    b.HasIndex("ProductId");

                    b.ToTable("ProductStatusHistory", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.ProductValues", b =>
                {
                    b.Property<Guid>("ProductValuesId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid?>("AttributeId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("AttributeOptionId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<double?>("Value")
                        .HasColumnType("double precision");

                    b.HasKey("ProductValuesId");

                    b.HasIndex("AttributeId");

                    b.HasIndex("ProductId");

                    b.ToTable("ProductValues", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Products", b =>
                {
                    b.Property<Guid>("ProductId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateOnly?>("AssignedAt")
                        .HasColumnType("date");

                    b.Property<Guid>("BrandId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("CategoryId")
                        .HasColumnType("uuid");

                    b.Property<string>("CollectionUnitId")
                        .HasColumnType("text");

                    b.Property<DateOnly?>("CreateAt")
                        .HasColumnType("date")
                        .HasColumnName("CreateAt");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("PackageId")
                        .HasColumnType("text");

                    b.Property<Guid?>("PostId")
                        .HasColumnType("uuid");

                    b.Property<string>("QRCode")
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("Status");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<bool>("isChecked")
                        .HasColumnType("boolean");

                    b.HasKey("ProductId");

                    b.HasIndex("BrandId");

                    b.HasIndex("CategoryId");

                    b.HasIndex("CollectionUnitId");

                    b.HasIndex("CreateAt");

                    b.HasIndex("PackageId");

                    b.HasIndex("PostId")
                        .IsUnique();

                    b.HasIndex("QRCode")
                        .IsUnique();

                    b.HasIndex("UserId");

                    b.ToTable("Products", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.PublicHoliday", b =>
                {
                    b.Property<Guid>("PublicHolidayId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Description")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateOnly>("EndDate")
                        .HasColumnType("date");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date");

                    b.HasKey("PublicHolidayId");

                    b.ToTable("PublicHoliday", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Rank", b =>
                {
                    b.Property<Guid>("RankId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("IconUrl")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double>("MinCo2")
                        .HasColumnType("double precision");

                    b.Property<string>("RankName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.HasKey("RankId");

                    b.ToTable("Rank", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Role", b =>
                {
                    b.Property<Guid>("RoleId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("RoleId");

                    b.ToTable("Role", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Shifts", b =>
                {
                    b.Property<string>("ShiftId")
                        .HasColumnType("text");

                    b.Property<Guid>("CollectorId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("Shift_End_Time")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("Shift_Start_Time")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Vehicle_Id")
                        .HasColumnType("text");

                    b.Property<DateOnly>("WorkDate")
                        .HasColumnType("date");

                    b.HasKey("ShiftId");

                    b.HasIndex("CollectorId");

                    b.HasIndex("Vehicle_Id");

                    b.ToTable("Shifts", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.SystemConfig", b =>
                {
                    b.Property<Guid>("SystemConfigId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("CollectionUnitId")
                        .HasColumnType("text");

                    b.Property<string>("CompanyId")
                        .HasColumnType("text");

                    b.Property<string>("DisplayName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("GroupName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Key")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Value")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("SystemConfigId");

                    b.HasIndex("CollectionUnitId");

                    b.HasIndex("CompanyId");

                    b.ToTable("SystemConfig", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.User", b =>
                {
                    b.Property<Guid>("UserId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AppleId")
                        .HasColumnType("text");

                    b.Property<string>("Avatar")
                        .HasColumnType("text");

                    b.Property<string>("CollectionUnitId")
                        .HasColumnType("text");

                    b.Property<string>("CollectorCode")
                        .HasColumnType("text");

                    b.Property<string>("CompanyId")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreateAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid?>("CurrentRankId")
                        .HasColumnType("uuid");

                    b.Property<string>("Email")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.Property<string>("Phone")
                        .HasColumnType("text");

                    b.Property<double>("Points")
                        .HasColumnType("double precision");

                    b.Property<Guid>("RoleId")
                        .HasColumnType("uuid");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double>("TotalCo2Saved")
                        .HasColumnType("double precision");

                    b.HasKey("UserId");

                    b.HasIndex("CollectionUnitId");

                    b.HasIndex("CompanyId");

                    b.HasIndex("CreateAt");

                    b.HasIndex("CurrentRankId");

                    b.HasIndex("RoleId");

                    b.ToTable("User", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.UserAddress", b =>
                {
                    b.Property<Guid>("UserAddressId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double?>("Iat")
                        .HasColumnType("double precision");

                    b.Property<double?>("Ing")
                        .HasColumnType("double precision");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<bool>("isDefault")
                        .HasColumnType("boolean");

                    b.HasKey("UserAddressId");

                    b.HasIndex("UserId");

                    b.ToTable("UserAddress", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.UserDeviceToken", b =>
                {
                    b.Property<Guid>("UserDeviceTokenId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("FCMToken")
                        .HasColumnType("text");

                    b.Property<string>("Platform")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<string>("VoipToken")
                        .HasColumnType("text");

                    b.HasKey("UserDeviceTokenId");

                    b.HasIndex("UserId");

                    b.ToTable("UserDeviceToken", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.UserReport", b =>
                {
                    b.Property<Guid>("UserReportId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.PrimitiveCollection<List<string>>("Images")
                        .HasColumnType("text[]");

                    b.Property<Guid?>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<string>("ReportType")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("ResolveMessage")
                        .HasColumnType("text");

                    b.Property<DateTime?>("ResolvedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("UserReportId");

                    b.HasIndex("ProductId");

                    b.HasIndex("UserId");

                    b.ToTable("UserReport", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.UserToken", b =>
                {
                    b.Property<Guid>("UserTokenId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AccessToken")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("ExpiryDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Token")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("UserTokenId");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("UserToken", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.UserVoucher", b =>
                {
                    b.Property<Guid>("UserVoucherId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("IsUsed")
                        .HasColumnType("boolean");

                    b.Property<DateTime>("ReceivedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("UsedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("VoucherId")
                        .HasColumnType("uuid");

                    b.HasKey("UserVoucherId");

                    b.HasIndex("UserId");

                    b.HasIndex("VoucherId");

                    b.ToTable("UserVoucher", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Vehicles", b =>
                {
                    b.Property<string>("VehicleId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text");

                    b.Property<int>("Capacity_Kg")
                        .HasColumnType("integer");

                    b.Property<string>("CollectionUnit")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double>("Height_M")
                        .HasColumnType("double precision");

                    b.Property<double>("Length_M")
                        .HasColumnType("double precision");

                    b.Property<string>("Plate_Number")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Vehicle_Type")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double>("Width_M")
                        .HasColumnType("double precision");

                    b.HasKey("VehicleId");

                    b.HasIndex("CollectionUnit");

                    b.ToTable("Vehicles", (string)null);
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Voucher", b =>
                {
                    b.Property<Guid>("VoucherId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateOnly>("EndAt")
                        .HasColumnType("date");

                    b.Property<string>("ImageUrl")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double>("PointsToRedeem")
                        .HasColumnType("double precision");

                    b.Property<int>("Quantity")
                        .HasColumnType("integer");

                    b.Property<DateOnly>("StartAt")
                        .HasColumnType("date");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double>("Value")
                        .HasColumnType("double precision");

                    b.HasKey("VoucherId");

                    b.ToTable("Voucher", (string)null);
                });

            modelBuilder.Entity("CollectionOffDay", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.CollectionUnit", "CollectionUnits")
                        .WithMany()
                        .HasForeignKey("CollectionUnitId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("ElecWasteCollection.Domain.Entities.Company", "Company")
                        .WithMany()
                        .HasForeignKey("CompanyId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.Navigation("CollectionUnits");

                    b.Navigation("Company");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Account", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "User")
                        .WithMany("Accounts")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_Account_User");

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.AttributeOptions", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Attributes", "Attribute")
                        .WithMany("AttributeOptions")
                        .HasForeignKey("AttributeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_AttributeOptions_Attribute");

                    b.Navigation("Attribute");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.BrandCategory", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Brand", "Brand")
                        .WithMany("BrandCategories")
                        .HasForeignKey("BrandId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_BrandCategory_Brand");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Category", "Category")
                        .WithMany("BrandCategories")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_BrandCategory_Category");

                    b.Navigation("Brand");

                    b.Navigation("Category");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Category", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Category", "ParentCategory")
                        .WithMany("SubCategories")
                        .HasForeignKey("ParentCategoryId")
                        .HasConstraintName("FK_Category_ParentCategory");

                    b.Navigation("ParentCategory");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CategoryAttributes", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Attributes", "Attribute")
                        .WithMany("CategoryAttributes")
                        .HasForeignKey("AttributeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_CategoryAttributes_Attribute");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Category", "Category")
                        .WithMany("CategoryAttributes")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_CategoryAttributes_Category");

                    b.Navigation("Attribute");

                    b.Navigation("Category");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CollectionGroups", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Shifts", "Shifts")
                        .WithMany("CollectionGroups")
                        .HasForeignKey("Shift_Id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_CollectionGroups_Shifts");

                    b.Navigation("Shifts");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CollectionRoutes", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.CollectionGroups", "CollectionGroup")
                        .WithMany("CollectionRoutes")
                        .HasForeignKey("CollectionGroupId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_CollectionRoutes_CollectionGroups");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Products", "Product")
                        .WithMany("CollectionRoutes")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_CollectionRoutes_Products");

                    b.Navigation("CollectionGroup");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CollectionUnit", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Company", "Company")
                        .WithMany("CollectionUnits")
                        .HasForeignKey("CompanyId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_CollectionUnit_CollectionCompany");

                    b.Navigation("Company");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CompanyRecyclingCategory", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Category", "Category")
                        .WithMany("CompanyRecyclingCategories")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_CompanyRecyclingCategory_Category");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Company", "Company")
                        .WithMany("CompanyRecyclingCategories")
                        .HasForeignKey("CompanyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_CompanyRecyclingCategory_Company");

                    b.Navigation("Category");

                    b.Navigation("Company");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.ForgotPassword", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "User")
                        .WithMany("ForgotPasswords")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_ForgotPassword_User");

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Image", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Post", "Post")
                        .WithMany("Images")
                        .HasForeignKey("PostId")
                        .HasConstraintName("FK_Images_Post");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Products", "Product")
                        .WithMany("Images")
                        .HasForeignKey("ProductId")
                        .HasConstraintName("FK_Images_Products");

                    b.Navigation("Post");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Notifications", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "User")
                        .WithMany("Notifications")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_Notification_User");

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.PackageStatusHistory", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Packages", "Packages")
                        .WithMany("PackageStatusHistories")
                        .HasForeignKey("PackageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_PackageStatusHistory_Packages");

                    b.Navigation("Packages");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Packages", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.CollectionUnit", "CollectionUnits")
                        .WithMany("Packages")
                        .HasForeignKey("CollectionUnitId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_Packages_CollectionUnits");

                    b.Navigation("CollectionUnits");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.PointTransactions", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Products", "Product")
                        .WithMany("PointTransactions")
                        .HasForeignKey("ProductId")
                        .HasConstraintName("FK_PointTransactions_Product");

                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "User")
                        .WithMany("PointTransactions")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_PointTransactions_User");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Voucher", "Voucher")
                        .WithMany("PointTransactions")
                        .HasForeignKey("VoucherId")
                        .HasConstraintName("FK_PointTransactions_Voucher");

                    b.Navigation("Product");

                    b.Navigation("User");

                    b.Navigation("Voucher");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Post", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.CollectionUnit", "AssignedCollectionUnit")
                        .WithMany("Posts")
                        .HasForeignKey("AssignedCollectionUnitId")
                        .HasConstraintName("FK_Post_CollectionUnits");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Company", "Company")
                        .WithMany("Posts")
                        .HasForeignKey("CompanyId")
                        .HasConstraintName("FK_Post_Company");

                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "Sender")
                        .WithMany("Posts")
                        .HasForeignKey("SenderId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_Post_User");

                    b.Navigation("AssignedCollectionUnit");

                    b.Navigation("Company");

                    b.Navigation("Sender");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.ProductStatusHistory", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Products", "Product")
                        .WithMany("ProductStatusHistories")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_ProductStatusHistory_Products");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.ProductValues", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Attributes", "Attribute")
                        .WithMany("ProductValues")
                        .HasForeignKey("AttributeId")
                        .HasConstraintName("FK_ProductValues_Attribute");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Products", "Product")
                        .WithMany("ProductValues")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_ProductValues_Products");

                    b.Navigation("Attribute");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Products", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Brand", "Brand")
                        .WithMany("Products")
                        .HasForeignKey("BrandId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_Products_Brand");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Category", "Category")
                        .WithMany("Products")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_Products_Category");

                    b.HasOne("ElecWasteCollection.Domain.Entities.CollectionUnit", "CollectionUnits")
                        .WithMany("Products")
                        .HasForeignKey("CollectionUnitId")
                        .HasConstraintName("FK_Products_CollectionUnits");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Packages", "Package")
                        .WithMany("Products")
                        .HasForeignKey("PackageId")
                        .HasConstraintName("FK_Products_Packages");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Post", "Post")
                        .WithOne("Product")
                        .HasForeignKey("ElecWasteCollection.Domain.Entities.Products", "PostId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("FK_Product_Post");

                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "User")
                        .WithMany("Products")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_Products_User");

                    b.Navigation("Brand");

                    b.Navigation("Category");

                    b.Navigation("CollectionUnits");

                    b.Navigation("Package");

                    b.Navigation("Post");

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Shifts", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "Collector")
                        .WithMany("Shifts")
                        .HasForeignKey("CollectorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_Shifts_User");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Vehicles", "Vehicle")
                        .WithMany("Shifts")
                        .HasForeignKey("Vehicle_Id")
                        .HasConstraintName("FK_Shifts_Vehicles");

                    b.Navigation("Collector");

                    b.Navigation("Vehicle");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.SystemConfig", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.CollectionUnit", "CollectionUnits")
                        .WithMany("CustomSettings")
                        .HasForeignKey("CollectionUnitId")
                        .HasConstraintName("FK_SystemConfig_CollectionUnits");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Company", "Company")
                        .WithMany("CustomSettings")
                        .HasForeignKey("CompanyId")
                        .HasConstraintName("FK_SystemConfig_Company");

                    b.Navigation("CollectionUnits");

                    b.Navigation("Company");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.User", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.CollectionUnit", "CollectionUnits")
                        .WithMany("Users")
                        .HasForeignKey("CollectionUnitId")
                        .HasConstraintName("FK_User_CollectionUnits");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Company", "Company")
                        .WithMany("Users")
                        .HasForeignKey("CompanyId")
                        .HasConstraintName("FK_User_Company");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Rank", "Rank")
                        .WithMany("User")
                        .HasForeignKey("CurrentRankId")
                        .HasConstraintName("FK_User_Rank");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Role", "Role")
                        .WithMany("Users")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_User_Role");

                    b.Navigation("CollectionUnits");

                    b.Navigation("Company");

                    b.Navigation("Rank");

                    b.Navigation("Role");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.UserAddress", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "User")
                        .WithMany("UserAddresses")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_UserAddress_User");

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.UserDeviceToken", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "User")
                        .WithMany("UserDeviceTokens")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_UserDeviceToken_User");

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.UserReport", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.Products", "Product")
                        .WithMany("UserReports")
                        .HasForeignKey("ProductId")
                        .HasConstraintName("FK_UserReport_Product");

                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "User")
                        .WithMany("UserReports")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_UserReport_User");

                    b.Navigation("Product");

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.UserToken", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "User")
                        .WithOne("UserToken")
                        .HasForeignKey("ElecWasteCollection.Domain.Entities.UserToken", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_UserToken_User");

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.UserVoucher", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.User", "User")
                        .WithMany("UserVouchers")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_UserVoucher_User");

                    b.HasOne("ElecWasteCollection.Domain.Entities.Voucher", "Voucher")
                        .WithMany("UserVouchers")
                        .HasForeignKey("VoucherId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_UserVoucher_Voucher");

                    b.Navigation("User");

                    b.Navigation("Voucher");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Vehicles", b =>
                {
                    b.HasOne("ElecWasteCollection.Domain.Entities.CollectionUnit", "CollectionUnits")
                        .WithMany("Vehicles")
                        .HasForeignKey("CollectionUnit")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_Vehicles_CollectionUnits");

                    b.Navigation("CollectionUnits");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Attributes", b =>
                {
                    b.Navigation("AttributeOptions");

                    b.Navigation("CategoryAttributes");

                    b.Navigation("ProductValues");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Brand", b =>
                {
                    b.Navigation("BrandCategories");

                    b.Navigation("Products");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Category", b =>
                {
                    b.Navigation("BrandCategories");

                    b.Navigation("CategoryAttributes");

                    b.Navigation("CompanyRecyclingCategories");

                    b.Navigation("Products");

                    b.Navigation("SubCategories");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CollectionGroups", b =>
                {
                    b.Navigation("CollectionRoutes");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.CollectionUnit", b =>
                {
                    b.Navigation("CustomSettings");

                    b.Navigation("Packages");

                    b.Navigation("Posts");

                    b.Navigation("Products");

                    b.Navigation("Users");

                    b.Navigation("Vehicles");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Company", b =>
                {
                    b.Navigation("CollectionUnits");

                    b.Navigation("CompanyRecyclingCategories");

                    b.Navigation("CustomSettings");

                    b.Navigation("Posts");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Packages", b =>
                {
                    b.Navigation("PackageStatusHistories");

                    b.Navigation("Products");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Post", b =>
                {
                    b.Navigation("Images");

                    b.Navigation("Product")
                        .IsRequired();
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Products", b =>
                {
                    b.Navigation("CollectionRoutes");

                    b.Navigation("Images");

                    b.Navigation("PointTransactions");

                    b.Navigation("ProductStatusHistories");

                    b.Navigation("ProductValues");

                    b.Navigation("UserReports");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Rank", b =>
                {
                    b.Navigation("User");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Role", b =>
                {
                    b.Navigation("Users");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Shifts", b =>
                {
                    b.Navigation("CollectionGroups");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.User", b =>
                {
                    b.Navigation("Accounts");

                    b.Navigation("ForgotPasswords");

                    b.Navigation("Notifications");

                    b.Navigation("PointTransactions");

                    b.Navigation("Posts");

                    b.Navigation("Products");

                    b.Navigation("Shifts");

                    b.Navigation("UserAddresses");

                    b.Navigation("UserDeviceTokens");

                    b.Navigation("UserReports");

                    b.Navigation("UserToken");

                    b.Navigation("UserVouchers");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Vehicles", b =>
                {
                    b.Navigation("Shifts");
                });

            modelBuilder.Entity("ElecWasteCollection.Domain.Entities.Voucher", b =>
                {
                    b.Navigation("PointTransactions");

                    b.Navigation("UserVouchers");
                });
#pragma warning restore 612, 618
        }
    }
}
